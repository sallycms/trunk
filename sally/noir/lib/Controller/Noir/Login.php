...
class sly_Controller_Noir_Login extends sly_Controller_Noir implements sly_Controller_Generic {
	...
	public function loginAction() {
		...
		if ($loginOK === true) {
			...
			$loginOK = $user->isAdmin() || $user->hasRight('apps', 'noir');
		}
		...
		if ($loginOK !== true) {
			...
			if ($referer && !sly_Util_String::startsWith(basename($referer), 'index.php?page=login')) {
				...
				$url  = $base.'/noir/index.php?page='.$user->getStartPage();
